...
function cwlpl_wp_redirect( $location, $status ) {
	...
	if(!is_user_logged_in()) {
		...
		if(trim((string)wp_parse_url($location, PHP_URL_PATH ), '/') === cwlpl_login_slug()) {
			$parsed_query = array();
...
function cwlpl_login_url_plugins_loaded() {
    ...
    // Check if REQUEST_URI is set and parse Requested URI
    if ( isset( $_SERVER['REQUEST_URI'] ) ) {
        // Unslash the REQUEST_URI
        $request_uri = wp_unslash( $_SERVER['REQUEST_URI'] );

        $URI  = wp_parse_url( $request_uri );
        ...
        // wp-register.php
        elseif ( !is_admin() && ( strpos( rawurldecode( $request_uri ), 'wp-register.php' ) !== false || strpos( rawurldecode( $request_uri ), 'wp-signup.php' ) !== false || $path === site_url( 'wp-register', 'relative' ) ) ) {
            // Set Flag
            $cwlpl_wp_login = true;
...
function cwlp_posotion_style() {
	if ( get_option( 'cwlploginposition' ) === '0' ) {
		...
		echo '<style>#login {margin: 0 0 auto auto !important;}.language-switcher {text-align: right!important;}</style>';
	}
}
if ( ! wp_is_mobile() ) {
