...
function cwlp_styles() {
	...
	if ( $cwlp_stylees === '0' ) {
		echo '<style>#simplecaptcha{width:100%}.cwlp-num{font-family: vazir;}</style>';
	...
	if ( ! empty( $cwlp_custom_logo ) ) {
		echo '<style>body.login div#login h1 a {background-image: url(' . esc_url( $cwlp_custom_logo ) . ');}</style>';
	}
	if ( empty( $cwlp_imbcu ) && ! empty( $cwlp_imbc ) ) {
		echo '<style>body {background-image: url(' . esc_url( $cwlp_imbc ) . ') !important;height: 100% !important;background-position: center !important;background-repeat: no-repeat !important;-webkit-background-size: cover !important;-moz-background-size: cover !important; -o-background-size: cover !important;background-size: cover !important;}</style>';
	}
	if ( ! empty( $cwlp_imbcu ) ) {
		echo '<style>body {background-image: url(' . esc_url( $cwlp_imbcu ) . ') !important;height: 100% !important;background-position: center !important;background-repeat: no-repeat !important;-webkit-background-size: cover !important;-moz-background-size: cover !important; -o-background-size: cover !important;background-size: cover !important;}</style>';
	}
...
function cwlp_custom_inline() {
	$style_c = '.login .message, .login .notice, .login .success { color: #000; }button#reload-captcha {background: transparent;border: none;display: flex;}.login form .forgetmenot {margin-top: 7px;}.g-recaptcha iframe {width: 100%;}';
...
function cwlpl_disable_login_url() {
    ...
    $message = !empty($cwlplmes) ? $cwlplmes : esc_html__('You do not have permission to access this page!', 'cwlp');

    if ( $cwlpllsstyle === '0' ) {
        wp_die( '<pre>' . wp_kses_post($message) . '</pre><div id="cwlp-copyright"><p><a href="https://wpirani.ir/" target="_blank">' . esc_html__('Protected By WP Irani', 'cwlp') . '</p></div><style>@font-face {font-family: vazir; src:url(' . esc_url( CWLP_LOGIN_ASSETS ) . 'fonts/vazir.woff2); font-style:  normal; font-display: swap;} body, h1, h2, pre, p, div {font-family: vazir; font-size: 1.2rem;} #cwlp-copyright p {text-align: center;}</style>' );
    } elseif ( $cwlpllsstyle === '1' ) {
        wp_die( '<pre>' . wp_kses_post($message) . '</pre><div id="cwlp-copyright"><div class="cwlpcopright"><p><a href="https://wpirani.ir/" target="_blank">' . esc_html__('Protected By WP Irani', 'cwlp') . '</p></div></div><link rel="stylesheet" href="' . esc_url( CWLP_LOGIN_ASSETS ) . 'css/cwlp-login-security-one.css" type="text/css" media="all" />' );
    } elseif ( $cwlpllsstyle === '2' ) {
        wp_die( '<pre>' . wp_kses_post($message) . '</pre><div id="cwlp-copyright"><div class="cwlpcopright"><p><a href="https://wpirani.ir/" target="_blank">' . esc_html__('Protected By WP Irani', 'cwlp') . '</p></div></div><link rel="stylesheet" href="' . esc_url( CWLP_LOGIN_ASSETS ) . 'css/cwlp-login-security-two.css" type="text/css" media="all" />' );
    }
}
...
function creative_wordpress_login_page_set() {
    ...
    if ( strpos( $current_page, 'cwlp-settings' ) === 0 ) {
        // Enqueue the appropriate styles
        if ( is_rtl() ) {
            wp_register_style( 'cwlp-sett', CWLP_LOGIN_ASSETS . 'css/cwlp-setings-rtl.css', array(), CWLP_VER );
        } else {
            wp_register_style( 'cwlp-sett', CWLP_LOGIN_ASSETS . 'css/cwlp-setings.css', array(), CWLP_VER );
        }
	...
	?>
		<style>li#toplevel_page_cwlp-settings a img {width: 25px !important;height: 25px !important;opacity: 1 !important;}li#toplevel_page_cwlp-settings .wp-menu-name {color: #ffffff;background: #2196f3;transition: background 0.2s linear 0.2s;}li#toplevel_page_cwlp-settings .wp-menu-name:hover {color: #ffffff;background: #e91e1e;transition: background 0.2s linear 0.2s;}body.toplevel_page_cwlp-settings {background: #fff;}</style>
	<?php
}
